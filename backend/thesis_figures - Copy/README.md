...
Shows accuracy performance for all 51 users across all 3 algorithms in a color-coded heatmap:
...
│
├── accuracy_heatmap_all_users.png     # Heatmap - 51 users × 3 algorithms
├── best_model_summary.png             # Pie + Bar chart summary
└── top_15_users.png                   # Top performers ranking
```

Total: **54 high-resolution figures** (300 DPI, PNG format)
- **51 individual user charts**
- **3 summary/overview charts**
...
> **Figure X**: Performance comparison of three ML algorithms for User 53 (DSL User s002).
> MLPClassifier achieved 96.11% accuracy with 5.83% EER, outperforming OneClassSVM
> (69.44% accuracy) and IsolationForest (5.00% accuracy). Gold outline indicates the
> selected model.

### For Heatmap:
> **Figure X**: Accuracy heatmap showing performance of three ML algorithms across 51 users.
> Green indicates high accuracy (>90%), yellow medium (50-90%), and red low (<50%).
> MLPClassifier consistently achieved the highest accuracy across all users.
...
**Algorithms**: OneClassSVM, IsolationForest, MLPClassifier
